...
inline void processGoal(byte mask, bool *goal, unsigned long *goalTime, unsigned long *offTime, const char *goalString) {
  byte state = getGoalState();
  static char l[100];
  if ((state & mask) > 0) {
    if (!*goal) {
      *goal = true;
...
void loop() {
  if (testMode) {
    ...
    if (goalsEnabled) {
      processGoal(PIN_MASK_YELLOW, &goalY, &goalTimeY, &offTimeY, GOAL_YELLOW_STR);
      processGoal(PIN_MASK_BLACK,  &goalB, &goalTimeB, &offTimeB, GOAL_BLACK_STR);
    }
